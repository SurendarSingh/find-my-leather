...
const OrderDocuments = ({ UserOrderDetails }: any) => {
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>, orderId: string, field: string) => {
        if(e.target.files){
    ...
    function getDocumentLink(orderId: string, fieldName: string, file: File): Promise<string>{
        return new Promise(async (resolve, reject)=> {
            if(file){
                const pathName  = `documents/${orderId}/${fieldName}/${file.name + uuidv4()}.pdf`;
    ...
    function renderDocument(documents: any, id: any) {
        return <>
            {Object?.keys(documents).map(key => {
                return <>
                    <td className="">{(documents && documents[key]) ? (
                        <div className="flex flex-col items-center justify-center p-2">
                            <a target="_blank" href={documents[key]} className="p-2"> 
                                <FaEye />
                            </a>
                            <a download  type="application/octet-stream" href={documents[key]} className="p-1"> <FaDownload /></a>
